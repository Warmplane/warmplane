        ...
        <div style="flex: 1; display: grid; grid-template-columns: 1fr 1fr; overflow: hidden;">
          ...
          <div style="padding: 16px; border-right: 1px solid var(--border); overflow-y: auto;">
            <div class="form-group">
              ...
              <textarea class="form-textarea" rows="8" id="pg-args-input">${escapeHtml(initialArgs)}</textarea>
            </div>
            <div class="form-group">
            ...
            ${selectedCap && selectedCap.input_schema ? `
              <div style="margin-top: 14px;">
                ...
                <pre style="background: var(--surface); padding: 10px; border-radius: var(--radius-sm); border: 1px solid var(--border); font-size: 11px; color: var(--text-muted); max-height: 180px; overflow-y: auto;">${escapeHtml(JSON.stringify(selectedCap.input_schema, null, 2))}</pre>
              </div>
            ` : ''}
          </div>
